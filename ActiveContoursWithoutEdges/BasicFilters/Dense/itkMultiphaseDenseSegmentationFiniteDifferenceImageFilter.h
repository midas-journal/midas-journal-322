...
/**
 * \class DenseSegmentationFiniteDifferenceImageFilter
 *
...
class ITK_EXPORT DenseSegmentationFiniteDifferenceImageFilter
  : public MultiphaseFiniteDifferenceImageFilter<TInputImage, TOutputImage >
{
public:
  /** Standard class typedefs */
  typedef DenseSegmentationFiniteDifferenceImageFilter Self;
...
Superclass;
  ...
  itkTypeMacro( DenseSegmentationFiniteDifferenceImageFilter,
    ImageToImageFilter );
...
protected:
  DenseSegmentationFiniteDifferenceImageFilter()
  {
		reinitCounter = 1;
		updateCounter = 0;
	}
  ...
  void SetFunctionCount( unsigned int n )
  {
    ...
    m_UpdateBuffers = new UpdateBufferPtr[n];
    for( unsigned int i = 0; i < this->FunctionCount; i++ )
    {
      m_UpdateBuffers[i] = UpdateBufferType::New();
    }
  }
  ...
  unsigned int reinitCounter;
  unsigned int updateCounter;

private:
  DenseSegmentationFiniteDifferenceImageFilter(const Self&);
  ...
  typedef typename UpdateBufferType::Pointer UpdateBufferPtr;

  typedef BinaryThresholdImageFilter< OutputImageType, OutputImageType >
    ThresholdFilterType;
  typedef SignedMaurerDistanceMapImageFilter< OutputImageType, OutputImageType >
    MaurerType;

  UpdateBufferPtr *m_UpdateBuffers;
};
...
#include "itkMultiphaseDenseSegmentationFiniteDifferenceImageFilter.txx"
#endif
